...
from utils import EarlyStopping, MaskCriterion
...
class Opt:
    ...
    # - data config
    caption_file = r"./msvd/captions.json"  # the file generated in prepare_captions.py
    feats_path = (
        r"./msvd/video_features/"  # the features extracted by extract_features.py
    )
    # - model config
    train_length = (
        80  # fix length during training, the feats length must be equal to this
    )
    dim_hidden = 512
    dim_embed = 512
    feat_dim = 2048
...
def save_opt(opt):
    with open(
        os.path.join(opt.save_path, opt.start_time + "opt.txt"), "w+", encoding="utf-8"
    ) as f:
...
def train():
    ...
    test_loader = torch.utils.data.DataLoader(
        testset, batch_size=opt.batch_size, shuffle=False
    )
    ...
    try:
        for epoch in range(opt.EPOCHS):
            ...
            # writer.add_scalar(
            #     "lr", optimizer.state_dict()["param_groups"][0]["lr"], global_step=epoch
            # )
            ...
            if epoch % opt.save_freq == 0:
                ...
                torch.save(
                    model,
                    os.path.join(opt.save_path, opt.start_time + str(epoch) + ".pth"),
                )
