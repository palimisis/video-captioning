...
class S2VT(nn.Module):
    ...
    def forward(
        self, feats, targets=None, mode="train", beam_width=3, max_beam_depth=30
    ):
        """
        ...
        if mode == "beam_search":
            ...
            padding = torch.zeros(
                [batch_size, self.length, self.dim_embed],
                dtype=torch.long,
                device=device,
            )
            ...
            return self.beam_search(
                state1, state2, beam_width=beam_width, max_depth=max_beam_depth
            )

        # pad_feats [B, 2L-1, dim_hid]
        padding = torch.zeros(
            [batch_size, self.length - 1, self.dim_hid], device=device
        )
        ...
        if mode == "train":
            ...
            padding = torch.zeros(
                [batch_size, self.length, self.dim_embed], device=device
            )
            ...
            input2 = torch.cat(
                [sos, output1[:, self.length, :].unsqueeze(dim=1)], dim=2
            )
            # output2 [B, 1, hid]
            ...
            for i in range(self.length - 2):
                ...
                input2 = torch.cat(
                    [input2, output1[:, self.length + i + 1, :].unsqueeze(dim=1)], dim=2
                )
                # [B, 1, 2hid] -> [B, 1, hid]
                output2, state2 = self.word_rnn(input2, state2)
    ...
    def load_glove_weights(
        self, glove_path, glove_dim, ix2word, word2embed="./data/word2embed.json"
    ):
        ...
        weights = torch.zeros(
            [self.vocab_size, glove_dim], dtype=torch.float, device=torch.device("cuda")
        )
        torch.nn.init.xavier_normal_(weights)
        for ix, word in ix2word.items():
            if word in word2embed:
                single_word_embed = torch.tensor(
                    word2embed[word], dtype=torch.float, device=torch.device("cuda")
                )
    ...
    def beam_search(self, state1, state2, beam_width=3, max_depth=30):
        ...
        """
        state1 = state1[0].transpose(dim0=0, dim1=1), state1[1].transpose(
            dim0=0, dim1=1
        )
        state2 = state2[0].transpose(dim0=0, dim1=1), state2[1].transpose(
            dim0=0, dim1=1
        )
        ...
        for batch in tqdm(range(batch_size), desc="dealing with sentences"):
            ...
            vid_state = (
                state1[batch][0].unsqueeze(dim=0),
                state1[batch][1].unsqueeze(dim=0),
            )
            word_state = (
                state2[batch][0].unsqueeze(dim=0),
                state2[batch][1].unsqueeze(dim=0),
            )
            ...
            while depth_count < max_depth:
                ...
                for score, n in beam_nodes:
                    ...
                    decoder_input = torch.cat(
                        [embed_word.view([1, 1, -1]), vid_out], dim=2
                    )
