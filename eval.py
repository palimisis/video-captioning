...
class Opt:
    model_path = r"./checkpoint/23_11_07_21_29_19-final.pth"
    csv_file = r"./msvd/video_corpus.csv"
    train_source_file = r"./msvd/annotation2016/train_val_videodatainfo.json"
    caption_file = r"./msvd/captions.json"
    feats_path = r"./msvd/video_features"
    batch_size = 10
...
def eval():
    ...
    valid_loader = torch.utils.data.DataLoader(
        validset, batch_size=opt.batch_size, shuffle=False
    )
...
def pred_to_coco_samples_IDs(prediction_dict, gts=None):
    ...
    for item in prediction_dict.items():
        if gts is not None and item[0] in gts:
            IDs.append(item[0])
            samples[item[0]] = [{"image_id": item[0], "caption": item[1]}]
    return samples, IDs
...
if __name__ == "__main__":
    ...
    with open("./msvd/gts.json", encoding="utf-8") as f:
        gts = json.load(f)["gts"]
