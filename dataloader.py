...
class VideoDataset(Dataset):
    def __init__(self, captions_file, feat_path, max_len=80, mode="train"):
        ...
        print(
            "prepare {} dataset. vocab_size: {}, dataset_size: {}".format(
                mode, len(self.word2ix), len(self.feat_paths)
            )
        )
    ...
    def __getitem__(self, index):
        ...
        # Padd features to the same length
        # feat = feat.unsqueeze(0) 
        feat = torch.nn.functional.pad(feat, (0, 0, 0, self.max_len - feat.shape[0]))

        labels = np.array(self.captions[ID], dtype=object)
